...
const mapInputToModelFeatures = (data) => {
  ...
  return [
    ...
    slopeValue,
    data.majorVessels || 0,
    data.thalassemia === 'Normal'
      ? 3
      : data.thalassemia === 'Fixed defect'
        ? 6
        : data.thalassemia === 'Reversible defect'
          ? 7
          : 3, // Default to 3 if not provided
  ];
};
...
const predictCardiovascularDisease = async (data) => {
  try {
    ...
    if (resultPercentage < 50) cardiovascularRisk = 'Low';
    else cardiovascularRisk = 'High Risk';
