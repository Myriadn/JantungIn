  ...
  <div class="result-page">
    <div class="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div class="mb-6 flex justify-between items-center">
        <button @click="goBack" class="flex items-center text-blue-600 hover:text-blue-700 transition-colors">
        ...
        <button @click="showSendToPatient" class="flex items-center px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <!-- Main Content -->
      <div class="bg-white rounded-lg shadow-lg p-6">        <!-- Header Section with Result -->
        <div class="flex flex-col lg:flex-row justify-between items-center mb-10">
          ...
          <!-- Result Circle -->
          <div class="flex flex-col items-center bg-white p-6 rounded-lg shadow-lg border border-gray-100">
            <div class="relative">
              <svg class="w-40 h-40" viewBox="0 0 36 36">
                <!-- Background circle -->
                <circle cx="18" cy="18" r="15.9" fill="none" stroke="#e0e0e0" stroke-width="2.5"></circle>
              ...
              <div class="absolute inset-0 flex flex-col items-center justify-center">
                <span class="text-4xl font-bold" :class="textColorClass">{{ resultPercentage }}%</span>
                <span class="text-sm font-semibold tracking-wider" :class="textColorClass">{{ resultText }}</span>
              </div>
            </div>
        ...
        <div class="mb-10 p-6 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <ul class="space-y-2">
                <li v-for="(factor, index) in topRiskFactors" :key="index" class="flex items-center">
            ...
            <div>
              ...
              <ul class="space-y-2">
                <li v-for="(recommendation, index) in recommendations" :key="index" class="flex items-center">
        ...
        <div class="mb-10">
          ...
          <div class="overflow-x-auto">
            <table class="w-full border-collapse">
              <thead>
                <tr class="bg-gray-50">
                  <th class="px-4 py-3 text-left text-sm font-medium text-gray-700 border border-gray-200">Feature</th>
                  <th class="px-4 py-3 text-left text-sm font-medium text-gray-700 border border-gray-200">Value</th>
                  <th class="px-4 py-3 text-left text-sm font-medium text-gray-700 border border-gray-200">Normal Range/Description</th>
                  <th class="px-4 py-3 text-left text-sm font-medium text-gray-700 border border-gray-200">Status</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(item, index) in patientDataTable" :key="index" 
                    class="hover:bg-gray-50" 
                    :class="{'bg-red-50': item.highlight}">
                  <td class="px-4 py-3 text-sm border border-gray-200">{{ item.feature }}</td>
                  <td class="px-4 py-3 text-sm font-medium border border-gray-200" 
                      :class="item.highlight ? 'text-red-600' : ''">
                    {{ item.value }}
                  </td>
                  <td class="px-4 py-3 text-sm text-gray-500 border border-gray-200">{{ item.normalRange }}</td>
        ...
        <div class="mb-10">
          ...
          <div class="grid grid-cols-1 gap-6">
            ...
            <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
              <div class="flex justify-between mb-4">
                <div>
                  ...
                  <p class="text-gray-600 text-sm">How each factor affects your heart disease risk</p>
                </div>
              ...
              <div class="space-y-6">
                ...
                <div class="relative h-6 mb-4 mt-2">
                  ...
                  <div class="absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-2 h-6 bg-gray-400 rounded-sm"></div>
                  
                  <div class="absolute left-0 -bottom-1 text-xs text-gray-500">Decreases Risk</div>
                  <div class="absolute right-0 -bottom-1 text-xs text-gray-500 text-right">Increases Risk</div>
                </div>
                ...
                <div v-for="(item, index) in featureContributions" :key="index" class="relative">
                  <div class="flex justify-between mb-2">
                    <div class="flex items-center">
                      <div class="w-6 h-6 rounded-full flex items-center justify-center mr-2"
                        ...
                        <svg v-if="item.value > 0" xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <div class="h-3 bg-gray-200 rounded-full relative">
                    <!-- Center line -->
                    <div class="absolute h-full w-px bg-gray-400 left-1/2 transform -translate-x-1/2"></div>
              ...
              <div class="mt-8 pt-4 border-t border-gray-200">
                ...
                <ul class="space-y-2 text-sm">
                  <li v-for="(factor, index) in topRiskFactors" :key="index" class="flex items-start">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-red-500 mr-2 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                    <span>{{ factor }}</span>
                  </li>
            ...
            <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
              ...
              <p class="mb-4 text-gray-600 text-sm">Relative impact of positive factors on diagnosis</p>
              ...
              <div class="flex flex-col md:flex-row items-center justify-between">
                ...
                <div class="donut-chart-container w-52 h-52 relative">
                  <svg viewBox="0 0 100 100" class="donut-chart">
                    ...
                    <circle class="donut-ring" cx="50" cy="50" r="40" fill="transparent" stroke="#e2e8f0" stroke-width="15"></circle>
                    ...
                    <!-- Center Text -->
                    <text x="50" y="45" text-anchor="middle" class="donut-text font-bold" font-size="10">
                ...
                <div class="mt-4 md:mt-0 md:ml-6">
                  <div class="space-y-3">
                    <div v-for="(item, index) in positiveContributions" :key="index" 
                        class="flex items-center">
                      <div :style="{ backgroundColor: donutColors[index % donutColors.length] }"
                          class="w-3 h-3 rounded-sm mr-2"></div>
        ...
        <div class="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-6 mt-10">
          <button 
        ...
        <!-- NIK Input Dialog -->
        <div v-if="showNikDialog" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div class="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
            <h3 class="text-lg font-semibold mb-4">Enter Patient NIK</h3>
            <p class="text-gray-600 mb-4">Please enter the patient's NIK (National ID Number) to save this diagnosis to their history.</p>
            
            <div class="mb-4">
              <label for="patientNik" class="block text-sm font-medium text-gray-700 mb-1">NIK</label>
              <input
                type="text"
                id="patientNik"
                v-model="patientNik"
        ...
        <div v-if="showSendDialog" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div class="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
            <h3 class="text-lg font-semibold mb-4">Send Results to Patient</h3>
            <p class="text-gray-600 mb-4">Enter the patient's information to send this diagnosis report.</p>
            
            <div class="mb-4">
              <label for="sendPatientNik" class="block text-sm font-medium text-gray-700 mb-1">NIK</label>
              <input
                type="text"
                id="sendPatientNik"
            ...
            <div class="mb-4">
              <label for="sendPatientName" class="block text-sm font-medium text-gray-700 mb-1">Full Name</label>
              <input
                type="text"
                id="sendPatientName"
...
const patientDataTable = ref([
  ...
  { feature: 'Resting Blood Pressure', value: '145 mm/Hg', normalRange: '90-120 mm/Hg', highlight: true },
  { feature: 'Serum Cholesterol', value: '240 mg/dl', normalRange: '<200 mg/dl', highlight: true },
  { feature: 'Fasting Blood Sugar', value: '<120 mg/dl', normalRange: '<100 mg/dl', highlight: false },
